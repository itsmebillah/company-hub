...
export async function resolvePermanentAttendanceSelfie(
  job: {
    attachment_id: string;
    source_object_path: string;
    drive_file_id: string | null;
  },
  permanent: AttendancePermanentStorage,
  cache: AttendanceSelfieStorage,
) {
...
export const AttendanceMediaSyncService = {
  async run(input: { syncLimit?: number; cleanupLimit?: number } = {}) {
    ...
    const syncJobs = await AttendanceMediaSyncRepository.claimSync(
      workerId,
      input.syncLimit ?? 10,
    );
    ...
    for (const job of syncJobs) {
      try {
        const stored = await resolvePermanentAttendanceSelfie(
          job,
          permanent,
          cache,
        );
        ...
        console.error("[AttendanceMediaSyncService] Sync failed.", {
          attachmentId: job.attachment_id,
          safeError,
        });
        await AttendanceMediaSyncRepository.failSync(
          job.outbox_id,
          workerId,
          safeError,
        );
    ...
    const cleanupJobs = await AttendanceMediaSyncRepository.claimCleanup(
      workerId,
      input.cleanupLimit ?? 20,
    );
    result.cleanupClaimed = cleanupJobs.length;
    for (const job of cleanupJobs) {
      try {
        await permanent.verify(job.drive_file_id);
        if (await cache.exists(job.source_object_path))
          await cache.remove(job.source_object_path);
        if (await cache.exists(job.source_object_path))
          throw new Error("Cache deletion verification failed.");
        await AttendanceMediaSyncRepository.completeCleanup(
          job.attachment_id,
          workerId,
        );
        result.cleaned += 1;
        ...
        console.error("[AttendanceMediaSyncService] Cleanup failed.", {
          attachmentId: job.attachment_id,
          safeError,
        });
        await AttendanceMediaSyncRepository.failCleanup(
          job.attachment_id,
          workerId,
          safeError,
        );
