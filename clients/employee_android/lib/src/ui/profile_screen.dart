...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> pickPhoto() async { final file = await ImagePicker().pickImage(source: ImageSource.gallery, maxWidth: 1200, imageQuality: 82); if (file == null || !mounted) return; final bytes = await file.readAsBytes(); if (bytes.length > 5 * 1024 * 1024) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Photo must be 5 MB or smaller.'))); return; } setState(() => uploading = true); final ok = await widget.controller.uploadProfilePhoto(bytes: bytes, filename: file.name, contentType: file.mimeType ?? 'image/jpeg'); if (mounted) { setState(() => uploading = false); ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(ok ? 'Profile photo updated.' : widget.controller.profileError ?? 'Photo upload failed.'))); } }
