...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> pickPhoto() async { final file = await ImagePicker().pickImage(source: ImageSource.gallery, maxWidth: 1200, imageQuality: 82); if (file == null || !mounted) return; final bytes = await file.readAsBytes(); if (!mounted) return; final messenger = ScaffoldMessenger.of(context); if (bytes.length > 5 * 1024 * 1024) { messenger.showSnackBar(const SnackBar(content: Text('Photo must be 5 MB or smaller.'))); return; } setState(() => uploading = true); final ok = await widget.controller.uploadProfilePhoto(bytes: bytes, filename: file.name, contentType: file.mimeType ?? 'image/jpeg'); if (mounted) { setState(() => uploading = false); messenger.showSnackBar(SnackBar(content: Text(ok ? 'Profile photo updated.' : widget.controller.profileError ?? 'Photo upload failed.'))); } }
  @override Widget build(BuildContext context) { final p = widget.controller.profile; final s = widget.controller.session!.profile; final name = p?.name ?? s.name; return ListView(key: const Key('profileScreen'), padding: const EdgeInsets.all(20), children: [Center(child: p?.photoUrl?.isNotEmpty == true ? CircleAvatar(radius: 42, backgroundImage: NetworkImage(p!.photoUrl!)) : CircleAvatar(key: const Key('profileAvatar'), radius: 42, child: Text(_initials(name)))), TextButton.icon(onPressed: uploading ? null : pickPhoto, icon: const Icon(Icons.camera_alt_outlined), label: Text(uploading ? 'Uploading...' : 'Change photo')), const SizedBox(height: 8), Text(name, textAlign: TextAlign.center, style: Theme.of(context).textTheme.headlineSmall), Text(p?.roleName ?? s.roleName, textAlign: TextAlign.center), const SizedBox(height: 24), Card(child: Padding(padding: const EdgeInsets.all(16), child: editing ? _form(context) : _view(p, s))),]); }
  Widget _view(ProfileState? p, SessionProfile s) => Column(children: [ListTile(title: const Text('Employee ID'), subtitle: Text(p?.employeeId ?? s.employeeId, key: const Key('profileEmployeeId'))), ListTile(title: const Text('Role'), subtitle: Text(p?.roleName ?? s.roleName, key: const Key('profileRole'))), ListTile(title: const Text('Date of birth'), subtitle: Text(p?.dateOfBirth.isNotEmpty == true ? p!.dateOfBirth : 'Not set')), ListTile(title: const Text('Phone'), subtitle: Text(p?.phone.isNotEmpty == true ? p!.phone : 'Not set')), FilledButton.icon(key: const Key('editProfileButton'), onPressed: begin, icon: const Icon(Icons.edit), label: const Text('Edit profile'))]);
