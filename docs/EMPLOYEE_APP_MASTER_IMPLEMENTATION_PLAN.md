...
| Profile                  | Full employee details and editable approved fields                                                                       | View/edit name, employee ID, role, phone, email, date of birth; native photo picker/upload with fallback avatar                                                                     | `GET/PATCH/POST /api/mobile/v1/profile`; POST validates image and writes `${authUserId}/avatar.{ext}` to `profile-photos` via server admin, then updates employee photo_url | Employee/profile schema exists | **MIGRATED / NOT RELEASED** | Authenticated employee/company scope; JPG/PNG/WebP, 5 MB server limit; Flutter/Android tests blocked by tooling | `8eb62f6` | P0 | Native dependency requires `flutter pub get` before client build |
...
| Attendance selfie        | Conditional capture, validation, temporary private cache | Camera capture, preview/retake, policy-gated upload and attendance evidence path | `POST /api/mobile/v1/attendance/selfie` uses bearer auth and existing AttendanceSelfieService | `0043` attachment/outbox model exists | **IMPLEMENTED / UNRELEASED** | Flutter policy/model/controller/UI coverage; device validation pending | Current worktree | P0 | Reuses existing Drive outbox; no schema change |
...
| Notification center      | Employee-scoped latest notifications and unread/read state in the web header/navigation                                  | Home and navigation unread badges plus Updates list; authenticated mobile mark-read adapter and Flutter tap/refresh wiring                                                            | Dashboard reads plus `PATCH /api/mobile/v1/notifications/:id` through scoped `NotificationRepository`                    | Existing notification tables/realtime RLS unchanged                                                                | **PARTIALLY MIGRATED / NOT RELEASED**             | Employee/company scope enforced; realtime/push remain deferred                                                        | `8f53284` | P2       | Use refresh/reconciliation until realtime is explicitly supported                                               |
...
| Settings/password        | Password change and PWA-specific settings shell                                                                          | Native More/settings screen with theme toggle, notification status, account summary, app version, and logout; password/profile mutations remain unavailable without mobile adapters       | Native settings UI only; password/profile services remain web-only                                                          | No new schema needed for password                                                                                  | **PARTIAL / NOT RELEASED**                       | Theme toggle and logout widget coverage pending; web-only profile/password API gap remains                             | Dashboard parity checkpoint | P1       | Add authenticated mobile profile/password adapters before exposing edits                                      |
...
1. **Dashboard feature milestone:** Quick Links, employee-scoped unread updates,
   ...
   celebration/holiday card are implemented. Native footer now mirrors the Web
   Hub/Updates/Me/More grouping, and More provides supported theme, notification,
...
3. **Mobile API coverage:** leave, full Resources/Knowledge Hub, notification
   mutations/realtime delivery, full calendar, and password flows still
   require thin mobile routes around existing services.
   Profile read/update is now available through the authenticated mobile adapter
   for phone, email, and date of birth; native photo selection/upload remains
   blocked pending an approved picker/storage adapter.
...
Implement the mobile profile read/update adapter and secure profile-photo upload
pipeline, then add Flutter edit controls for the Web-approved phone, email,
date-of-birth, and photo fields. Keep protected identity fields read-only. Do not
bundle attendance selfie, tracking, release work, or backend deployment into that
checkpoint.
