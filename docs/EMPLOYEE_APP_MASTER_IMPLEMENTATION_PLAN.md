...
| Profile                  | Full employee details and editable approved fields                                                                       | View/edit name, employee ID, role, phone, email, date of birth; current photo display and fallback avatar                                                                         | `GET/PATCH /api/mobile/v1/profile` derives authenticated employee and company; updates only phone/email/date_of_birth       | Employee/profile schema exists                                                                                     | **PARTIALLY MIGRATED / NOT RELEASED**            | Server validation and tenant ownership; Flutter edit flow added; photo upload still requires approved native picker path | `d58735b` | P0 | Protected identity fields remain read-only; add secure photo upload next |
...
| Notification center      | Employee-scoped latest notifications and unread/read state in the web header/navigation                                  | Home and navigation unread badges plus Updates list; authenticated mobile mark-read adapter added                                                                                | Dashboard reads plus `PATCH /api/mobile/v1/notifications/:id` through scoped `NotificationRepository`                    | Existing notification tables/realtime RLS unchanged                                                                | **PARTIALLY MIGRATED / NOT RELEASED**             | Employee/company scope enforced; Flutter wiring and realtime/push remain deferred                                      | Final parity checkpoint | P2       | Use refresh/reconciliation until realtime is explicitly supported                                               |
